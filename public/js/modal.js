...
movieContainerCollectionEl.on('click', '.movie-card', function (e) {
    // Displayed the modal
    ...
    $('#remove-from-collection-btn').attr('data-id', movieId);
    $('#remove-from-collection-btn').removeAttr("disabled");
    $('#remove-from-collection-btn').text('REMOVE FROM MY COLLECTION');
});

movieContainerSearchEl.on('click', '.movie-card', function (e) {
    // Displayed the modal
    UIkit.modal(modalEl).show();
    // Path to the target card element
    target = $(e)[0].currentTarget
...
// Click event for the modal
movieSlideshowEl.on('click', '.slideshow-movie', function (e) {
    // Displayed the modal
    UIkit.modal(modalEl).show();
    // Path to the target card element
    target = $(e)[0].currentTarget
